...
for i in range(n_dies):
    _, _, _, energia, altitud = calcular_produccio_horaria(orbita_theta[i])
...
# dades dies extrems
hores_dia_max, pot_dia_max, irr_dia_max, _, _ = calcular_produccio_horaria(orbita_theta[dia_max])
hores_dia_min, pot_dia_min, irr_dia_min, _, _ = calcular_produccio_horaria(orbita_theta[dia_min])
...
plt.tight_layout()
plt.savefig('altitud_solar.png', dpi=300)
plt.show()
...
plt.ylim(bottom=0)
plt.tight_layout()
plt.savefig('produccio_diaria.png', dpi=300)
plt.show()
...
def plot_dia_detall(dia, energia, hores, pot, irr, color_pot, color_irr, tipus):
    ...
    plt.tight_layout()
    plt.savefig(f'potencia_dia_{tipus.lower()}.png', dpi=300)
    plt.show()
